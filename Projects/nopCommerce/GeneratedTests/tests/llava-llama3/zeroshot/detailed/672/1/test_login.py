...
class TestLoginScenario(unittest.TestCase):
    def test_login_scenario(self):
        ...
        email_field = WebDriverWait(self.driver, 20).until(
            presence_of_element_located((By.XPATH, "//input[contains(@name='Email')]"))
        )
        email_field.send_keys("admin@admin.com")
        password_field = WebDriverWait(self.driver, 20).until(
            presence_of_element_located((By.XPATH, "//input[contains(@name='Password')]"))
        )
        password_field.send_keys("admin")
        
        # Step 5: Click the login button.
        login_button = WebDriverWait(self.driver, 20).until(
            presence_of_element_located((By.XPATH, "//button[contains(@class='login-button')]"))
        )
        login_button.click()
