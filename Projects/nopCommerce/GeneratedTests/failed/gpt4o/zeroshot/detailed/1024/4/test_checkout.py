...
class CheckoutProcessTest(unittest.TestCase):
    ...
    def test_user_checkout_process(self):
        ...
        search_link = self.wait.until(element_to_be_clickable((By.LINK_TEXT, "Search")))
        search_link.click()

        # Search for a product using the query "book"
        search_input = self.wait.until(element_to_be_clickable((By.CLASS_NAME, "search-box-text")))
        search_input.send_keys("book")
        search_button = self.wait.until(element_to_be_clickable((By.CLASS_NAME, "search-box-button")))
        search_button.click()

        # Add the first result to the cart
        add_to_cart_button = self.wait.until(element_to_be_clickable((By.CLASS_NAME, "product-box-add-to-cart-button")))
        ...
        shopping_cart_link = self.wait.until(element_to_be_clickable((By.LINK_TEXT, "shopping cart")))
        shopping_cart_link.click()

        # Check the "Terms of service" and click "Checkout"
        terms_of_service_checkbox = self.wait.until(element_to_be_clickable((By.ID, "termsofservice")))
        terms_of_service_checkbox.click()
        checkout_button = self.wait.until(element_to_be_clickable((By.ID, "checkout")))
        checkout_button.click()

        # Choose "Checkout as Guest"
        checkout_as_guest_button = self.wait.until(element_to_be_clickable((By.CLASS_NAME, "checkout-as-guest-button")))
        checkout_as_guest_button.click()

        # Fill out the billing form
        fname_input = self.wait.until(element_to_be_clickable((By.ID, "BillingNewAddress_FirstName")))
        ...
        shipping_continue_button = self.wait.until(element_to_be_clickable((By.CSS_SELECTOR, "#shipping-method-buttons-container .button-1")))
        shipping_continue_button.click()

        # Payment method step
        payment_method_option = self.wait.until(element_to_be_clickable((By.CSS_SELECTOR, "#paymentmethod_1"))).click()
        ...
        # Payment info step
        cardholder_input = self.wait.until(element_to_be_clickable((By.ID, "CardholderName")))
        ...
        # Confirm order step
        confirm_button = self.wait.until(element_to_be_clickable((By.CSS_SELECTOR, "#confirm-order-buttons-container .button-1")))
