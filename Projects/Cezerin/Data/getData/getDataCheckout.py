...
try:
    ...
    try:
        checkout_button = popup.find_element(By.XPATH, "//a[contains(@class, 'button') and contains(text(), 'Go to checkout')]")
        checkout_button.click()
        driver.execute_script("document.body.style.zoom='50%'")
        time.sleep(3)
        save_html("checkout_before_filling")
        ...
        save_html("checkout_after_filling")
        driver.find_element(By.XPATH, "//button[contains(text(), 'Next')]").click()
        time.sleep(3)
        save_html("checkout_shipping")
        driver.find_element(By.XPATH, "//button[contains(text(), 'Place Order')]").click()
        time.sleep(3)
        save_html("checkout_final")
