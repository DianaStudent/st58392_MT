...
class CheckoutProcessTest(unittest.TestCase):
    ...
    def test_user_checkout_process(self):
        ...
        # Click on login
        login_button = wait.until(element_to_be_clickable((By.XPATH, "//a[text()='Login']")))
        login_button.click()

        # Enter login credentials
        email_field = wait.until(element_to_be_clickable((By.NAME, "username")))
        ...
        cart_icon = wait.until(element_to_be_clickable((By.XPATH, "//button[@class='icon-cart']")))
        cart_icon.click()
        
        # Wait for popup to become visible and click "View Cart"
        view_cart_button = wait.until(element_to_be_clickable((By.LINK_TEXT, "View Cart")))
        view_cart_button.click()

        # On the cart page, click "Proceed to Checkout"
        proceed_to_checkout = wait.until(element_to_be_clickable((By.LINK_TEXT, "Proceed to Checkout")))
        proceed_to_checkout.click()

        # Fill out the billing form
        company_field = wait.until(element_to_be_clickable((By.NAME, "company")))
